...
public class CORSWithCredentialsTest {
	...
	public void assertNoCORSNoOptions() throws MalformedURLException {
		...
		given().contentType(ContentType.JSON)
				...
				.log().ifError()
				.when().get(new URL(context, SearchRestServiceTest.SEARCH_REST_API).toExternalForm());
	}
	...
	public void assertOptions() throws MalformedURLException {
		given().contentType(ContentType.JSON)
				...
				.log().ifError()
				.statusCode(200)
				.header(ACCESS_CONTROL_ALLOW_ORIGIN, is(ORIGIN_VALUE))
				.header(ACCESS_CONTROL_ALLOW_CREDENTIALS, is("true"))
				.header(ACCESS_CONTROL_ALLOW_HEADERS, notNullValue())
				.header(ACCESS_CONTROL_ALLOW_METHODS, notNullValue())
				.header(ACCESS_CONTROL_MAX_AGE, notNullValue())
				.when().options(new URL(context, SearchRestServiceTest.SEARCH_REST_API).toExternalForm());
	}
	...
	public void assertCORS() throws MalformedURLException {
		Response response = given().contentType(ContentType.JSON)
				...
				.statusCode(200)
				.header(ACCESS_CONTROL_ALLOW_CREDENTIALS, is("true"))
				.header(ACCESS_CONTROL_ALLOW_HEADERS, nullValue())
				.header(ACCESS_CONTROL_ALLOW_METHODS, nullValue())
				.header(ACCESS_CONTROL_MAX_AGE, nullValue())
				.when().get(new URL(context, SearchRestServiceTest.SEARCH_REST_API).toExternalForm())
				.thenReturn();
	...
	protected void testCORSHeader(Response response) {
		...
		Assert.assertArrayEquals(
				validOrigin,
				response.getHeaders().getValues(ACCESS_CONTROL_ALLOW_ORIGIN).toArray());
	}
	...
	public void assertCORSOnESProxy() throws MalformedURLException {
		Response response = given().contentType(ContentType.JSON)
				.header(HEADER_ORIGIN, ORIGIN_VALUE)
				.header(HEADER_USER_AGENT, "Mozilla")
				...
				.header(ACCESS_CONTROL_ALLOW_METHODS, nullValue())
				.header(ACCESS_CONTROL_MAX_AGE, nullValue())
				.when().get(new URL(context, DeploymentHelpers.DEFAULT_REST_VERSION + "sys/es/search/").toExternalForm())
				.thenReturn();
		testCORSHeader(response);
	}
